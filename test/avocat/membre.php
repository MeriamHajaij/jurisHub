<?php
include 'db.php';
$sql = "SELECT * FROM members_du_barreau ORDER BY nom, prenom";
...
<head>
    ...
    <title>Annuaire des Avocats | JurisHub</title>
    ...
    <style>
        ...
        * {
        ...
        .avocat-card:nth-child(1) { animation-delay: 0.1s; }
        .avocat-card:nth-child(2) { animation-delay: 0.2s; }
        .avocat-card:nth-child(3) { animation-delay: 0.3s; }
        .avocat-card:nth-child(4) { animation-delay: 0.4s; }
        .avocat-card:nth-child(5) { animation-delay: 0.5s; }
        .avocat-card:nth-child(6) { animation-delay: 0.6s; }
        .avocat-card:nth-child(7) { animation-delay: 0.7s; }
        .avocat-card:nth-child(8) { animation-delay: 0.8s; }
...
<body>
    ...
    <aside class="sidebar">
        ...
        <div class="nav-container">
            <ul class="nav-links">
                ...
                <li class="nav-item">
                    <a href="membre.php" class="nav-link  active">
                        <i class="fas fa-users"></i>
    ...
    <main class="main-content">
        <div class="page-header">
            <h1 class="page-title">Annuaire des Avocats</h1>
            <p class="page-description">
                Consultez la liste complète des membres du barreau. Trouvez des collègues par spécialité ou localisation.
            </p>
        </div>
        ...
        <div class="search-filter-container">
            <div class="search-bar">
                ...
                <input type="text" id="search" placeholder="Rechercher par nom, prénom ou spécialité..." onkeyup="filterAvocats()">
            </div>
            
            <div class="filter-select">
                <select id="specialty-filter" onchange="filterAvocats()">
                    <option value="">Toutes les spécialités</option>
                    <?php
                    include 'db.php';
                    $specialties_sql = "SELECT DISTINCT legal_specialty FROM members_du_barreau ORDER BY legal_specialty";
                    $specialties_result = $conn->query($specialties_sql);
                    
                    if ($specialties_result->num_rows > 0) {
                        while ($row = $specialties_result->fetch_assoc()) {
                            echo '<option value="' . htmlspecialchars($row['legal_specialty']) . '">' . htmlspecialchars($row['legal_specialty']) . '</option>';
                        }
                    }
                    ?>
                </select>
        ...
        <div class="grid-container" id="avocat-list">
            <?php 
            $sql = "SELECT * FROM members_du_barreau ORDER BY nom, prenom";
            $result = $conn->query($sql);
            ...
            if ($result->num_rows > 0): ?>
                <?php while ($row = $result->fetch_assoc()): 
                    ...
                    $specialty = htmlspecialchars($row['legal_specialty']);
                    ...
                    <div class="avocat-card" 
                        ...
                        <div class="card-body">
                            <div class="avocat-info">
                                ...
                                <p><i class="fas fa-briefcase"></i> <?= htmlspecialchars($row['barreau'] ?? 'Barreau non spécifié') ?></p>
                            </div>
            ...
            <?php else: ?>
                <div class="no-results">
                    <i class="fas fa-user-tie"></i>
                    <p>Aucun avocat trouvé dans l'annuaire pour le moment.</p>
                </div>
            <?php endif; 
            $conn->close();
            ?>
    ...
    <script>
        ...
        function filterAvocats() {
            ...
            const cards = document.querySelectorAll(".avocat-card");
            ...
            cards.forEach(card => {
                ...
                const matchesSearch = name.includes(searchInput) || specialty.includes(searchInput);
                const matchesSpecialty = specialtyFilter === "" || specialty === specialtyFilter.toLowerCase();
